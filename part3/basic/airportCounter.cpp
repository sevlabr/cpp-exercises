...
public:
  ...
  AirportCounter(TIterator begin, TIterator end) {
  	...
  	for (auto it = begin; it != end; ++it) {
  		...
  		airports[idx].second += 1;
  	}
  }

  size_t Get(TAirport airport) const {
  	size_t idx = static_cast<size_t>(airport);
  	return airports[idx].second;
  }
  ...
  Items GetItems() const {
  	Items data_copy;
  	for (size_t i = 0; i < size; ++i) {
  		data_copy[i].first = static_cast<TAirport>(i);
  		data_copy[i].second = airports[i].second;
  	}
  	return data_copy;
  }
