...
void TestCopyingAbility() {
	SimpleVector<string> target;
	string to_copy = "a";
	target.PushBack(to_copy);
	ASSERT_EQUAL(target.Size(), 1u);
	ASSERT_EQUAL(to_copy.size(), 1u);
	ASSERT_EQUAL(to_copy, "a");
	ASSERT_EQUAL(target[0], "a");

	string to_move = "b";
	target.PushBack(move(to_move));
	ASSERT_EQUAL(target.Size(), 2u);
	ASSERT_EQUAL(to_move.size(), 0u);
	ASSERT_EQUAL(target[0], "a");
	ASSERT_EQUAL(target[1], "b");
}
...
int main() {
  ...
  RUN_TEST(tr, TestCopyingAbility);
  return 0;
}
